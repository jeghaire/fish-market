...
export default async function Page({
  ...
  const { session_id: sessID } = searchParams;
  ...
  const products: IStripeProduct[] = await fetchLineItems(sessID);

  const subtotal =
    products &&
    products.reduce(
      (acc, product: IStripeProduct) => acc + product.price.unit_amount / 100,
      0
    );
